...
const CircularBuffer =
    @import("../CircularBuffer.zig").CircularBuffer;
...
fn autoInitializeCarrier(socket: network.Socket) !void {
    ...
    const Cluster = struct {
        ...
        direction: Direction,
        current_axis_initialize: usize,
    };
    var clusters =
        try CircularBuffer(Cluster).init(allocator, 1024);
    ...
    for (hall_sensors, 0..) |alarm_state, idx| {
        std.log.debug(
            "start_idx: {}, disabled: {}, enabled: {}",
            .{ start_index, disabled_sensor_count, enabled_sensor_count },
        );
        if (chain == .Disabled) {
            if (alarm_state == true) {
                chain = .Enabled;
                ...
                if (enabled_sensor_count != 0 and disabled_sensor_count == 3) {
                    ...
                    std.log.debug(
                        "hall_idx: {}, axis_idx: {} -> backward: {}, forward: {}",
                        .{ idx, idx / 2, backward_cluster, forward_cluster },
                    );
                    if (forward_cluster and backward_cluster and enabled_sensor_count <= 2) {
                        ...
                        try clusters.write(.{
                            ...
                            .end = end_index,
                            .direction = .backward,
                            .current_axis_initialize = start_index,
                        });
                        ...
                        enabled_sensor_count > 2)
                    {
                        ...
                        if (@mod(mean_idx, 2) == 0) {
                            ...
                            try clusters.write(.{
                                ...
                                .end = end_index,
                                .direction = .forward,
                                .current_axis_initialize = end_index,
                            });
                        } else {
                            try clusters.write(.{
                                .start = start_index,
    ...
    while (clusters.read()) |cluster| {
        ...
        const current_axis = cluster.current_axis_initialize / 2;
        ...
        const start_axis = cluster.start / 2;
        const end_axis = cluster.end / 2;
        std.log.debug(
            "cluster start: {}, cluster end: {}, current hall: {}",
            .{ cluster.start, cluster.end, cluster.current_axis_initialize },
        );
        std.log.debug(
            "cluster start: {}, cluster end: {}, axis index being checked: {}",
            .{ start_axis, end_axis, current_axis },
        );
        std.log.debug(
        ...
        if (try checkCarrierExistence(
            ...
            const target_axis = try getFreeAxisIndex(
                cluster.start,
                cluster.end,
                cluster.direction,
                socket,
            ...
            if ((cluster.current_axis_initialize < cluster.end - 3 and
                cluster.direction == .backward) or
                (cluster.current_axis_initialize > cluster.end + 3 and
                    cluster.direction == .forward))
            {
                try clusters.write(.{
                    .current_axis_initialize = if (cluster.direction == .backward)
                        cluster.current_axis_initialize + 2
                    else
                        cluster.current_axis_initialize - 2,
            ...
            if (try checkHallStatus(
                cluster.current_axis_initialize,
                socket,
            ) == false) {
                try clusters.write(.{
                    .current_axis_initialize = if (cluster.direction == .backward)
                        cluster.current_axis_initialize + 1
                    else
                        cluster.current_axis_initialize - 1,
                    .direction = cluster.direction,
                    .end = cluster.end,
            ...
            var param = std.mem.zeroes(mmc.ParamType(.set_command));
            var line_idx: usize = undefined;
            const axis_idx: usize = if (cluster.current_axis_initialize % 2 != 0)
                (cluster.current_axis_initialize - 1) / 2
            else
                cluster.current_axis_initialize / 2;
            for (starting_axis_indices, 0..) |starting_axis_index, idx| {
                if (axis_idx >= starting_axis_index) {
            ...
            param.link_axis = .no_direction;
            try sendMessage(
                .set_command,
                param,
                socket,
            );
            try clusters.write(cluster);
        } else {
            try clusters.write(cluster);
        }
        std.log.debug("\n\n", .{});
...
fn mapHallSensors(
    ...
    while (detected_active_axis < num_of_active_axis) {
        ...
        std.log.debug(
            "starting_axis_indices length: {} -> {}",
            .{ starting_axis_indices.len, hall_sensor.line_id * 2 - 1 },
        );
